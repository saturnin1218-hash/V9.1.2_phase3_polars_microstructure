...
from binance_quant_builder.api import build_native_execution_plan
...
def test_phase2_feature_only_with_liquidations_and_acceleration_backend(tmp_path: Path):
    ...
    assert run_manifest["native_mode"] == "feature-only"


def test_phase2_execution_plan_exposes_acceleration_backend(tmp_path: Path):
    cfg = AppConfig(
        run=RunConfig(
            symbol="BTCUSDT",
            out_dir=str(tmp_path),
    ...
    plan = build_native_execution_plan(cfg)
    assert plan["native_supplemental"]["acceleration_backend"] == "polars"
